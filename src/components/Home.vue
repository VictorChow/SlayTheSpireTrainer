<template>
  <div>
    ...
    <div class="flex-row">
      <div style="width:50%">
        ...
        <el-autocomplete 
          v-model="inputCard"
          clearable 
          :fetch-suggestions="getCardsSuggestions" 
          placeholder="请输入卡片名称"
     ...
     <div style="width:50%">
        ...
        <el-button type="primary" style="margin-left:30px" @click="addRelic">添加遗物</el-button>
        <el-button type="warning" style="margin-left:30px" @click="deleteSelectedRelics">删除选中</el-button>
    ...
    <el-button type="primary" style="margin-top:30px;margin-bottom:30px" @click="exportFile">生成存档密文</el-button>
...
export default {
  ...
  methods: {
    ...
    getCardsSuggestions(queryString, cb) {
      let array = queryString
        ? allPotions.filter(
            item => item.toLowerCase().indexOf(queryString.toLowerCase()) === 0
          )
        : allPotions;
    ...
    addRelic() {
      ...
      this.relics.push(this.inputRelic.trim());
      this.inputRelic = "";
    },
    exportFile() {
